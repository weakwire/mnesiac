defmodule Mnesiac do
  ...
  ```
  """
  @spec node_in_cluster?(node :: node()) :: true | false
  def node_in_cluster?(node), do: Enum.member?(:mnesia.system_info(:db_nodes), node)
  ...
  ```
  """
  @spec running_db_node?(node :: node()) :: true | false
  def running_db_node?(node) do
    {:ok, cluster} = running_nodes()
    Enum.member?(cluster, node)
  end

  defp filter_cluster(nodes), do: Enum.filter(Node.list(), fn node -> node in List.flatten(nodes) end)

  defp join_cluster(config, override, node) do
    with {:ok, config_struct} <- build_struct(config, override),
         ...
         :ok <- copy_tables(config_struct, node),
  ...
  defp connect(node) do
    with :ok <- ensure_remote_started(node),
         {:ok, [_node]} <- :mnesia.change_config(:extra_db_nodes, [node]) do
      :ok
    else
  ...
  defp ensure_remote_started(node) do
    with :ok <- :rpc.call(node, :mnesia, :start, []),
         :ok <- wait_for(:remote_start, node) do
      :ok
    else
      {:error, reason} -> {:error, reason}
    end
  ...
  defp copy_schema(config, node) do
    apply(config.schema.ref, :copy_schema, [config, node])
  end

  defp init_schema(config), do: apply(config.schema.ref, :init_schema, [config])

  defp copy_tables(config, node) do
    local_cookies = get_table_cookies()
    remote_cookies = get_table_cookies(node)
    ...
    Enum.each(config.stores, fn store ->
      ...
      case {local_cookies[cookie], remote_cookies[cookie]} do
        ...
        {_, nil} ->
          {:error, {:no_remote_records_to_copy, store.ref, node}}

        {_local, _remote} ->
          apply(store.ref, :resolve_conflict, [config, node])
      end
    end)
  end
  ...
  defp wait_for(:remote_start, node) do
