defmodule Mnesiac.Store do
  ...
  @typedoc """
  Default store configuration expected to be passed in to Mnesiac. Everything is optional except `ref`. `:disc_only_copies ` is silently dropped for `:schema`.
  """
  @type config ::
          {:ref, module() | atom()}
  ...
  **Note**: The default implementation is to do nothing.
  ...
  def copy_schema(config, cluster_node) do
    Enum.each(Map.from_struct(config.schema), fn {type, nodes} ->
      if type in [:ram_copies, :disc_copies] do
